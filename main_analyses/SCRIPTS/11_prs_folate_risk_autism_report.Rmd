...
save_as_docx(
  interact_genotype_compact_table,
  path = here("RESULTS", "table_3_interact_compact.docx")
)
```

```{r}
max_estimate_to_plot <- max(cur_data$estimate, na.rm = TRUE) %>%
  round()
cur_data %>%
  select(estimate:age, adj) %>%
  distinct() %>%
  filter(!is.na(estimate) & estimate < (max_estimate_to_plot + 1)) %>%
...
ggsave(filename = here("FIGURES", "autistic_risk_interaction_models.png"))
```
...
or_plot <- function(cur_data){
  cur_data %>%
    ...
    geom_pointrange(aes(xmin = conf.low, xmax = conf.high)) +
    scale_x_continuous(breaks = c(0, 1, 5, 10, 15, 20)) +
    coord_cartesian(expand = FALSE, xlim = c(0, 20)) +
    theme_minimal() +
    theme(
...
interact_genotype_tab_plot <- make_table2_plot_compact(
  data = cur_data,
  title = cur_title,
  levels = levels_autist
)
interact_genotype_tab_plot

save_as_docx(
  interact_genotype_tab_plot,
  path = here("RESULTS", "table_3_interact_with_plot.docx")
)
```
